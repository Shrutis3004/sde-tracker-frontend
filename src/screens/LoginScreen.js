...
export default function LoginScreen({ navigation }) {
  ...
  const handleLogin = async () => {
    ...
    catch (error) { Alert.alert('Error', error.response?.data?.error || 'Login failed'); }
    ...
    <View style={s.container}>
      ...
      <KeyboardAvoidingView style={s.content} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
        ...
        <View style={s.logoArea}>
          ...
          <Text style={s.logoTitle}>takeUforward</Text>
          <Text style={s.logoSub}>SDE Sheet Tracker</Text>
        </View>
        ...
        <View style={s.modal}>
          ...
          <TouchableOpacity onPress={handleLogin} disabled={loading}>
            <LinearGradient colors={['#3BA8D4', '#62c1e5', '#a0d9ef']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={s.loginBtn}>
...
const s = StyleSheet.create({
  ...
  loginBtn: { paddingVertical: 14, borderRadius: 12, alignItems: 'center', marginTop: 4 },
